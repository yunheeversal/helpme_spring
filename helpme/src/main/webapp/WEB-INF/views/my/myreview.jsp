...
<div class="online-course-category-1x  margin-bottom-large">
	<div class="container">
		<div class="row margin-top-middle padding-side-small">
			<h2 class="edufair-main-title font-pretendard" style="text-align: left">나의 리뷰</h2>

			<c:if test="${empty review}">
				<strong>등록한 리뷰가 없습니다.</strong>
				<br />
					...
					<div class="edufair-related-course">
						<div class="edufair-related-course-overlay">
							<%-- <img
								...
								<img src="displayFile?fileName=${review.getsAttach()}" class="edufair-shop-image">
						...
						<div class="related-course-content">
							<div class="media">
								...
								<div class="media-body">
									<a href="/service/detail?sNo=${review.getsNo()} " class="">
										<h4 class="media-heading">${review.getrTitle()}
											(${review.getsName()})</h4> <span>${review.getsCon()}</span>
									</a>
								</div>
							</div>
							<a href="/service/detail?sNo=${review.getsNo()} " class="">
								<h3>${review.getrContent()}</h3>
								
							</a>
							<div class="course-review">
								<p>
									<fmt:formatDate pattern="yyyy-MM-dd "
										value="${review.getrRegDate()}" />
								</p>
								<a href="#" class=""> <c:forEach var="i" begin="1"
										end="${review.getRating()}" step="1">
							...
							<div class="related-course-footer">
								<i class="fa fa-users"></i> <span><fmt:formatNumber
										value="${review.getViewCount()}" pattern="###,###,###" /></span> <i
									class="fa fa-heart"></i> <span><fmt:formatNumber
										value="${review.getLikeIt()}" pattern="###,###,###" /></span> <strong><fmt:formatNumber
										value="${review.getPrice()}" pattern="###,###,###" />원</strong>
							</div>
						</div>
					</div>
				</div>
					...
					<nav class="course-pagination" aria-label="Page navigation">
						<ul class="pagination edufair-pagination">
							<c:if test="${pageMaker.prev}">
								<li><a
									href="myreview${pageMaker.makeSearch1(pageMaker.startPage - 1) }&userId=${member.userId}"
							...
							<c:forEach begin="${pageMaker.startPage }"
								...
								<li
									...
									<a href="myreview${pageMaker.makeSearch1(idx)}&userId=${member.userId}">${idx}</a>
							</c:forEach>

							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a
									href="myreview${pageMaker.makeSearch1(pageMaker.endPage +1) }&userId=${member.userId}"
